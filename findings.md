...
| F-15 | Performance | LOW | Bundle | `package.json` (recharts ~400 kB, lucide-react) | Kein Bundle-Monitoring; recharts landete über die statisch importierte ObjectEditModal→HistoryTab→HistoryChart-Kette (sowie StateListModals→HistoryModal) im Initial-Chunk | Recharts (~430 kB) im Entry-Bundle, obwohl nur bei geöffnetem Chart gebraucht | `rollup-plugin-visualizer` in Build aufgenommen; ObjectEditModal + HistoryTab + HistoryModal via `lazy()` ausgelagert → recharts in eigenem Chunk. Index 1159→725 kB (gzip 326→200) | S | Performance | FIXED (12aa40e) |
...
9. **F-12** ~~`getStates?pattern=*`-Fallback lädt gesamte Installation~~ FIXED (HIGH)
...
- ~~**Bundle unbeobachtet** (F-15): recharts-Kette über nicht-lazy ObjectEditModal.~~ FIXED
...
- ~~`pattern=*`-Fallback durch Namespace-Patterns ersetzen (F-12)~~ FIXED
- API-Grenz-Validierung (F-17), E2E-Smoke-Test (F-20)
- ~~Bundle-Analyse + ObjectEditModal lazy (F-15)~~ FIXED
