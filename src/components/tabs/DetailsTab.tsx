...
function InlineInputRow({ label, value, onSave, isPending }: { label: string; value: string; onSave: (v: string) => void; isPending: boolean }) {
  ...
  return (
    <div className="flex gap-3 py-0.5 border-b border-gray-200 dark:border-gray-800 items-center">
...
function DetailRow({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="flex gap-3 py-1 border-b border-gray-200 dark:border-gray-800">
...
function SectionHeader({ label, first = false }: { label: string; first?: boolean }) {
  return (
    <div className={`flex items-center gap-2 ${first ? 'mb-1' : 'mt-3 mb-1'}`}>
...
export default function DetailsTab({
  ...
  const selectDropdownCls = 'w-full appearance-none [color-scheme:light] dark:[color-scheme:dark] [&>option]:bg-white [&>option]:text-gray-900 dark:[&>option]:bg-gray-800 dark:[&>option]:text-gray-200 bg-gray-50/70 text-gray-700 text-sm rounded-md pl-2.5 pr-8 py-1.5 border border-gray-200 focus:border-gray-300 focus:outline-none focus:ring-1 focus:ring-gray-300/70 disabled:opacity-50 dark:bg-gray-800/70 dark:text-gray-200 dark:border-gray-700 dark:focus:border-gray-600 dark:focus:ring-gray-600/60 transition-colors';
  const labelCls = 'text-gray-400 dark:text-gray-500 text-xs w-32 shrink-0 uppercase tracking-wide';
  const rowCls = 'flex gap-3 py-0.5 border-b border-gray-200 dark:border-gray-800 items-center';
    ...
    <div className="px-4 py-3 overflow-y-auto flex-1 flex flex-col gap-1">
      ...
      {state && (
        <div className="flex flex-col mb-2">
          <SectionHeader label={isEn ? 'Live value' : 'Aktueller Wert'} first />
          <div className={rowCls + ' py-1'}>
            <span className={labelCls}>{isEn ? 'Value' : 'Wert'}</span>
              ...
              {editValue ? (
                <>
                  {expertMode ? (
                    ...
                    <StringControl val={frozenVal} onSet={onSetValue} isPending={setValuePending} unit={obj.common?.unit} language={language} />
                  ) : (
                    <span className="font-mono text-gray-400 dark:text-gray-500 text-sm">{isEn ? 'Read-only' : 'Schreibgeschützt'}</span>
                  )}
                  <button
                    onClick={() => setEditValue(false)}
                    className="shrink-0 text-xs text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
                ...
                <>
                  ...
                  {(isWritable || expertMode || isButton) && (
                    <label className="inline-flex items-center gap-1.5 ml-2 cursor-pointer select-none shrink-0">
                      ...
                      <span className="text-xs text-gray-400 dark:text-gray-500">{isEn ? 'Set' : 'Setzen'}</span>
                    </label>
                  )}
                </>
              )}
      ...
      <div className="grid grid-cols-2 gap-x-5">
        ...
        {/* LEFT: Identity + Type & Role */}
        <div className="flex flex-col">

          {/* ── Identity ── */}
          <SectionHeader label={isEn ? 'Identity' : 'Identität'} first={!state} />
          ...
          <div className={rowCls}>
            <span className={labelCls}>{isEn ? 'Role' : 'Rolle'}</span>
            <div className="flex-1 relative">
              <select
                value={obj.common?.role || ''}
                onChange={(e) => saveField('role', e.target.value)}
                disabled={extendPending}
                className={`${selectDropdownCls} ${obj.common?.role ? getRoleColor(obj.common.role) : 'text-gray-700 dark:text-gray-200'}`}
              >
          ...
          {obj.type === 'state' && (
            <>
              <div className={rowCls}>
                <span className={labelCls}>{isEn ? 'Value type' : 'Werttyp'}</span>
                <div className="flex-1 relative">
                  <select
                    value={obj.common?.type || ''}
                    onChange={(e) => saveField('type', e.target.value)}
                    disabled={extendPending}
                    className={selectDropdownCls}
                  >
              ...
              <div className={rowCls}>
                <span className={labelCls}>{isEn ? 'Unit' : 'Einheit'}</span>
                <div className="flex-1 relative">
                  <select
        ...
        {/* RIGHT: Permissions & Range + Smart Home */}
        <div className="flex flex-col">

          {/* ── Permissions & Range (state only) ── */}
          {obj.type === 'state' && (
            <>
              <SectionHeader label={isEn ? 'Permissions & Range' : 'Berechtigungen & Bereich'} first={!state} />
              <div className={rowCls}>
                <span className={labelCls}>{isEn ? 'Read / Write' : 'Lesen / Schreiben'}</span>
          ...
          {/* ── Smart Home ── */}
          <SectionHeader label="Smart Home" first={obj.type !== 'state' && !state} />
          <div className={rowCls}>
            <span className={labelCls}>{isEn ? 'Room' : 'Raum'}</span>
            <div className="flex-1 relative">
              <select
                value={roomEnumId ?? ''}
          ...
          <div className={rowCls}>
            <span className={labelCls}>{isEn ? 'Function' : 'Funktion'}</span>
            <div className="flex-1 relative">
              <select
                value={fnEnumId ?? ''}
